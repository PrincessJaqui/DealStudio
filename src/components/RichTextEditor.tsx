  ...
  const editor = useEditor({
    extensions: [
      ...
      Link.configure({
        openOnClick: false,
        autolink: true,
        HTMLAttributes: { rel: 'noopener noreferrer', target: '_blank', class: 'text-[var(--ds-brand)] underline' },
      }),
    ],
  ...
  const ToolbarBtn = ({ onClick, active, title, children }: { onClick: () => void; active?: boolean; title: string; children: React.ReactNode }) => (
    <button
      ...
      className={`h-8 w-8 flex items-center justify-center rounded-md text-sm transition-colors ${
        active ? 'bg-[var(--ds-brand)] text-white' : 'text-[#364153] hover:bg-[#f3f4f6]'
      }`}
    >
      {children}
    </button>
  ...
  return (
    <div className="rounded-xl bg-[#f5f6f8] overflow-hidden focus-within:ring-2 focus-within:ring-[var(--ds-brand)]/30 transition-shadow flex flex-col">
      ...
      {showLinkInput && (
        <div className="flex items-center gap-2 px-2 py-2 border-b border-[#e6e9ec] bg-[#f5f6f8] shrink-0">
          <input
            ...
            className="flex-1 h-8 px-2 text-sm rounded-md border border-[#e5e7eb] focus:outline-none focus:border-[var(--ds-brand)]"
            autoFocus
          />
          <button type="button" onClick={handleAddLink} className="h-8 px-3 text-xs font-medium rounded-md bg-gradient-to-br from-[var(--ds-grad-from)] to-[var(--ds-grad-to)] text-white hover:bg-[var(--ds-brand)]">
      ...
      <style>{`
        ...
        .ProseMirror a { color: var(--ds-brand); text-decoration: underline; }
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .rich-text-content a { color: var(--ds-brand); text-decoration: underline; overflow-wrap: anywhere; word-break: break-word; }
