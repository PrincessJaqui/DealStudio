      ...
      <div className="flex items-start justify-between gap-3 mb-5">
        ...
        {admin && (
          <button type="button" onClick={() => onChange({ ...m, revenues: [...m.revenues, newRevenue()] })}
            className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-br from-[#5d8f41] to-[#76b252] px-4 py-2 text-sm font-semibold text-white shadow-sm shrink-0">
          ...
          {m.revenues.map((r, ri) => {
            ...
            return (
              <div key={r.id} className="self-start rounded-2xl border border-[#edf0f3] bg-white p-5 shadow-[0_4px_16px_-2px_rgba(0,0,0,0.06)]">
                ...
                <div className="mt-4 space-y-3">
                  {r.tiers.map((t, ti) => (
                    <div key={t.id} className="rounded-xl bg-[#f7f7f8] p-3">
                      <p className="text-[11px] font-bold uppercase tracking-wider text-[#5d8f41] mb-2">Pricing Tier</p>
                      ...
                      {t.impacts && (
                        <div className="mt-2 rounded-xl bg-[#eef6e6] border border-[#cce4b8] p-3">
                          <p className="text-[11px] font-bold uppercase tracking-wider text-[#5d8f41] mb-2">Impacted Tier</p>
                          ...
                          {admin && (
                            <button type="button" onClick={() => setTier(ri, ti, { impactedTiers: [...(t.impactedTiers || []), newImpacted()] })} className="mt-2 ml-auto block text-xs font-semibold text-[#5d8f41] hover:underline">+ Add More Impacted Tiers</button>
                          )}
                        </div>
                      )}
                    </div>
                ...
                {admin && (
                  <button type="button" onClick={() => setRev(ri, { tiers: [...r.tiers, newTier()] })} className="mt-3 ml-auto block text-xs font-semibold text-[#5d8f41] hover:underline">+ Add Pricing Tier</button>
                )}
                ...
                <div className="mt-4 border-t border-[#edf0f3] pt-3 space-y-2">
                  ...
                  <div className="flex items-center justify-between"><span className="text-sm font-semibold text-[#191f1d]">Annual Revenue</span><span className="text-sm font-bold text-[#5d8f41]">{fmtUsd(rMonthly * 12)}</span></div>
                </div>
              </div>
            );
          })}
    ...
    <>
      ...
      <div className="mt-6 grid gap-6 rounded-2xl border border-[#edf0f3] bg-white p-6 shadow-[0_4px_16px_-2px_rgba(0,0,0,0.06)] lg:grid-cols-[1.4fr_1fr]">
            ...
            {totals.revenues.map((r) => (
              <div key={r.id}>
                ...
                <div className="h-2 rounded-full bg-[#eef1f4] overflow-hidden">
                  <div className="h-full rounded-full bg-gradient-to-r from-[#5d8f41] to-[#76b252]" style={{ width: `${Math.min(100, r.pctOfTotal)}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between"><span className="text-sm text-[#7f8c85]">Monthly Revenue</span><span className="text-base font-bold text-[#5d8f41]">{fmtWhole(totals.totalMonthly)}</span></div>
            <div className="flex items-center justify-between"><span className="text-sm text-[#7f8c85]">Annual Revenue</span><span className="text-base font-bold text-[#5d8f41]">{fmtWhole(totals.totalAnnual)}</span></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              <tr><td className="py-2 pr-3 font-medium text-[#191f1d]">Total Revenue</td>{totals.growth.map(g => <td key={g.year} className={`py-2 px-3 text-right font-semibold ${g.year === 4 ? 'text-[#5d8f41]' : 'text-[#191f1d]'}`}>{fmtWhole(g.annual)}</td>)}</tr>
            </tbody>
          </table>
        </div>
      </div>
