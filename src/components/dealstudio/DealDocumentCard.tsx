...
export function DealDocumentCard({ doc, onOpen, admin = false, onEdit, onDelete, selectMode = false, selected = false, onToggleSelect, stat }: Props) {
  ...
  return (
    <div className={`ds-pulse ds-card group relative rounded-2xl border bg-white shadow-[0_4px_16px_-2px_rgba(0,0,0,0.06)] overflow-hidden ${selected ? 'border-[#76b252] ring-2 ring-[#76b252]/40' : 'border-[#edf0f3]'}`}>
      {/* Preview */}
      <button
        type="button"
          ...
          : (
            <span className="absolute inset-0 flex items-center justify-center bg-white">
              <span className="w-[88px] h-28 rounded-md bg-white shadow-[0_2px_10px_rgba(0,0,0,0.08)] border border-[#edf0f3] p-2.5 flex flex-col gap-1.5">
                <span className="h-2 w-3/4 rounded bg-[#76b252]/40" />
                <span className="h-1.5 w-full rounded bg-[#eef1f4]" />
                <span className="h-1.5 w-full rounded bg-[#eef1f4]" />
                <span className="h-1.5 w-5/6 rounded bg-[#eef1f4]" />
                <span className="mt-auto flex items-center gap-1 text-[#5d8f41]"><FileText className="w-3 h-3" /><span className="text-[8px] font-bold tracking-wide">FILE</span></span>
              </span>
            </span>
          )}
        {selectMode && (
          <span className={`absolute top-2 right-2 w-6 h-6 rounded-md flex items-center justify-center border-2 ${selected ? 'bg-[#5d8f41] border-[#5d8f41]' : 'bg-white/90 border-[#edf0f3]'}`}>
            {selected && <Check className="w-4 h-4 text-white" />}
          </span>
        )}
        {doc.is_deck && (
          <span className="absolute top-2 left-2 inline-flex items-center gap-1 rounded-full bg-gradient-to-br from-[#5d8f41] to-[#76b252] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white shadow">
      ...
      <div className="p-3">
        ...
        {admin && stat && (
          <div className="flex items-center gap-3 mt-2 pt-2 border-t border-[#f0f0f0] text-[11px] text-[#5d8f41] font-medium">
