...
export function ValuePropEditor({
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        <p className={labelCls}>The one line</p>
        <p className="text-xs text-[#9ca3af] mt-1 mb-2">
          If an investor remembers one sentence, make it this one.
        </p>
        <input
          className={input}
          value={v.headline}
          onChange={(e) => set({ headline: e.target.value })}
          placeholder="We help mid-market operators do X without Y."
        />
      </div>

      <div className={card}>
        <p className={labelCls}>The problem</p>
        <textarea
          className={`${input} mt-2 min-h-[92px] resize-y`}
          value={v.problem}
          onChange={(e) => set({ problem: e.target.value })}
          placeholder="What is broken today, and who feels it. Be concrete, not abstract."
        />
      </div>

      <div className={card}>
        <p className={labelCls}>Your solution</p>
        <textarea
          className={`${input} mt-2 min-h-[92px] resize-y`}
          value={v.solution}
