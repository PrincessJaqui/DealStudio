    ...
    <div>
      ...
      {(overflowing || expanded) && (
        <button
          ...
          className="mt-1 ml-auto block text-sm font-semibold text-[#5d8f41] hover:underline"
        >
      ...
      <a href={a.url} target="_blank" rel="noopener noreferrer" className="group block">
        ...
        <div className="p-3">
          <div className="flex items-start gap-2">
            {!showImage && (
              <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#f0f7ea] text-[#5d8f41]"><FileText className="w-4 h-4" /></span>
            )}
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-semibold text-[#191f1d] line-clamp-2 group-hover:text-[#5d8f41]">{a.title || 'Untitled'}</span>
              <span className="mt-0.5 block truncate text-xs text-[#99a1af]">{[a.source, a.date].filter(Boolean).join(' · ')}</span>
            </span>
            <ArrowUpRight className="w-4 h-4 shrink-0 text-[#cbd5cf] group-hover:text-[#5d8f41]" />
          </div>
        </div>
      </a>
      {hasDesc && (
        <div className="px-3 pb-3">
          ...
          <button type="button" onClick={() => setOpen(o => !o)} className="mt-1 ml-auto block text-xs font-semibold text-[#5d8f41] hover:underline">
    ...
    <div ref={ref} data-section="market" className={`${cardCls} ${inView ? 'ds-animate' : ''}`}>
      ...
      {anyMetric && (
        <>
            ...
            <svg viewBox="0 0 200 200" className="ds-pulse-pie h-44 w-44 shrink-0" aria-hidden="true">
              <defs>
                <linearGradient id="selGrad" x1="0" y1="0" x2="1" y2="1">
                  <stop offset="0%" stopColor="#3f6629" />
                  <stop offset="100%" stopColor="#5d8f41" />
                </linearGradient>
              </defs>
              <circle cx="100" cy="100" r="92" fill={active === 'tam' ? 'url(#selGrad)' : '#e9f2df'} className="cursor-pointer transition-all"
                onClick={() => setActive('tam')} />
              <circle cx="100" cy="100" r="64" fill={active === 'sam' ? 'url(#selGrad)' : '#c4deaa'} className="cursor-pointer transition-all"
                onClick={() => setActive('sam')} />
              <circle cx="100" cy="100" r="36" fill={active === 'som' ? 'url(#selGrad)' : '#a9d485'} className="cursor-pointer transition-all"
              ...
              {BANDS.map((b, bi) => {
                ...
                return (
                  <button
                    ...
                    className={`ds-pulse ds-card relative flex w-full items-center justify-between gap-3 rounded-2xl border px-5 py-4 text-left transition-colors ${on ? 'border-transparent bg-gradient-to-r from-[#5d8f41] to-[#76b252] shadow-[0_6px_20px_-6px_rgba(63,102,41,0.6)]' : 'border-[#edf0f3] bg-white hover:bg-[#f9fafb]'}`}
                  >
                    <span className="min-w-0">
                      <span className={`block text-[11px] font-bold uppercase tracking-wider ${on ? 'text-white' : 'text-[#5d8f41]'}`}>{b.label}</span>
                      <span className={`block text-xs ${on ? 'text-white/85' : 'text-[#7f8c85]'}`}>{b.full}</span>
                    </span>
                    <span className={`text-xl font-bold ${on ? 'text-white' : 'text-[#5d8f41]'}`}>{m?.value || '—'}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="mt-4 rounded-xl border border-[#edf0f3] bg-[#f9fafb] p-4">
            {metric?.note
    ...
    <div data-section="reading" className="rounded-2xl border border-[#edf0f3] bg-white shadow-[0_4px_16px_-2px_rgba(0,0,0,0.06)] p-5">
      ...
      {articles.length > 2 && (
        <button
          ...
          className="mt-3 ml-auto block text-sm font-semibold text-[#5d8f41] hover:underline"
        >
