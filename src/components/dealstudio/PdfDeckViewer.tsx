...
export function PdfDeckViewer({ url, className = '', onPageView }: Props) {
  ...
  const pillBtn = 'w-10 h-10 rounded-full bg-white border border-[#edf0f3] shadow-[0_2px_10px_rgba(0,0,0,0.12)] flex items-center justify-center text-[#191f1d] hover:border-[var(--ds-brand)] disabled:opacity-30 disabled:cursor-not-allowed transition-colors';
  ...
  const styleTag = (
    <style>{`
      ...
      .deck-next-pulse { animation: deckNextPulse 1.5s ease-in-out infinite; border-color: var(--ds-brand); }
  ...
  if (fullscreen) {
    return (
      <div ref={rootRef} tabIndex={0} className="fixed inset-0 z-[95] bg-black/95 flex flex-col items-center justify-center outline-none select-none">
        ...
        {loading && <span className="absolute inset-0 flex items-center justify-center"><Loader2 className="w-7 h-7 text-[var(--ds-brand)] animate-spin" /></span>}
