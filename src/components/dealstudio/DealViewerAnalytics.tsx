        ...
        <div className="p-5 space-y-5 overflow-y-auto">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={l} className="rounded-xl bg-[#f9fafb] border border-[#edf0f3] p-3">
                <p className="text-[10px] font-bold uppercase tracking-wide text-[#76b252]">{l}</p>
              ...
              <div className="space-y-2">
                {sections.map(([k, s]) => (
                  <div key={k} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-2 rounded-full bg-[#f0f7ea] overflow-hidden"><div className="h-full bg-gradient-to-r from-[#5d8f41] to-[#76b252]" style={{ width: `${Math.max(4, Math.round(((s as number) / maxSection) * 100))}%` }} /></div>
          ...
          {docSections.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {docSections.map(([k, s]) => (
                  <span key={k} className="text-[11px] text-[#5d8f41] bg-[#f0f7ea] rounded-full px-2.5 py-1">{label(k)} · {formatDuration(s as number)}</span>
                ))}
              </div>
            </div>
          )}
              ...
              <div className="space-y-2">
                {pages.map(p => (
                  <div key={p.page} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-2 rounded-full bg-[#f0f7ea] overflow-hidden"><div className="h-full bg-gradient-to-r from-[#5d8f41] to-[#76b252]" style={{ width: `${Math.max(4, Math.round((p.avgSeconds / maxPage) * 100))}%` }} /></div>
