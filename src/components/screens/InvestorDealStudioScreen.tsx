...
export function InvestorDealStudioScreen({ isMasterAdmin = false }: { isMasterAdmin?: boolean }) {
  ...
  if (loading) {
    return <div className="min-h-screen bg-[#f9fafb] flex items-center justify-center"><div className="w-8 h-8 border-2 border-[#76b252] border-t-transparent rounded-full animate-spin" /></div>;
  }
  if (!room) {
    return (
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb]">
      {isMasterAdmin && (
        <div className={`w-full text-center text-sm font-medium py-2 px-4 ${room.is_active ? 'bg-[#f0f7ea] text-[#5d8f41]' : 'bg-[#fff7ed] text-[#b45309]'}`}>
      ...
      <div className="max-w-6xl mx-auto p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
        ...
        <div className="contents lg:block lg:space-y-6 min-w-0">
          ...
          <div data-section="header" className="order-2 lg:order-none rounded-2xl border border-[#edf0f3] bg-white shadow-[0_4px_16px_-2px_rgba(0,0,0,0.06)] p-5">
            ...
            {room.headquarters && <p className="flex items-center gap-1 text-sm text-[#5d8f41] lg:mt-2"><MapPin className="w-4 h-4" /> {room.headquarters}</p>}
            {(room.tags?.length > 0 || room.industries?.length > 0) && (
              <div className="flex flex-wrap gap-2 mt-3">
                {room.tags?.map(t => <span key={t} className="rounded-full bg-[#f0f7ea] text-[#5d8f41] text-xs font-medium px-3 py-1">{t}</span>)}
          ...
          <div data-section="deal_info" className="order-4 lg:order-none rounded-2xl border border-[#edf0f3] bg-white shadow-[0_4px_16px_-2px_rgba(0,0,0,0.06)] p-5">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {tiles.map(t => (
                <div key={t.label} className="rounded-2xl bg-gradient-to-br from-[#5d8f41] to-[#76b252] p-4 text-white">
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    {hasMore && (
                      <button onClick={() => setAboutExpanded(e => !e)} className="mt-3 ml-auto block text-sm font-semibold text-[#5d8f41] hover:underline">
            ...
            {room.documents.length === 0 ? (
              ...
              <>
                ...
                {room.documents.length > 2 && (
                  <button onClick={() => setDocsExpanded(e => !e)} className="mt-3 ml-auto block text-sm font-semibold text-[#5d8f41] hover:underline">
        ...
        <div className="contents lg:block lg:space-y-6 lg:sticky lg:top-6">
          <div className="order-1 lg:order-none rounded-2xl border border-[#edf0f3] bg-white shadow-[0_4px_16px_-2px_rgba(0,0,0,0.06)] p-5 text-center">
            <div className="w-20 h-20 rounded-full bg-[#f5f7f9] border border-[#edf0f3] mx-auto mb-3 overflow-hidden flex items-center justify-center">
              {room.hero_image_url ? <img src={room.hero_image_url} alt="" className="w-full h-full object-cover" /> : <span className="text-2xl font-bold text-[#5d8f41]">{(room.company_name || '?').trim().charAt(0).toUpperCase()}</span>}
            </div>
            ...
            {room.meeting_enabled && (
              <Button onClick={() => setMeetingOpen(true)} className="w-full h-10 mt-3 rounded-full bg-gradient-to-br from-[#5d8f41] to-[#76b252] text-white hover:bg-[#648f47]">Schedule Meeting</Button>
            )}
            {room.allow_share && (
              <button onClick={shareDeal} className="w-full h-10 mt-2 rounded-full border border-[#76b252] text-[#5d8f41] hover:bg-[#f0f7ea] flex items-center justify-center gap-1.5 text-sm font-medium transition-colors">
          ...
          {room.meeting_enabled && (
            <div data-section="calendar" className="order-12 lg:order-none rounded-2xl border border-[#edf0f3] bg-white shadow-[0_4px_16px_-2px_rgba(0,0,0,0.06)] p-4">
              ...
              <Button onClick={() => setMeetingOpen(true)} className="w-full h-10 mt-3 rounded-full bg-gradient-to-br from-[#5d8f41] to-[#76b252] text-white hover:bg-[#648f47]">Request a meeting</Button>
