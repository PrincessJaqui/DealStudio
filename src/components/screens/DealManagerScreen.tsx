...
import { fetchOrgDeals, createDeal, dealUrl, type OrgDeal } from '../../lib/org';
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {deals.map((d) => (
            ...
            >
              ...
              {/* The full public URL, not the legacy path. This is the thing a
                  founder copies into an email, so showing them a stale /d/ link
                  they can no longer recognise was worse than showing nothing. */}
              <p className="text-xs text-[#7f8c85] mt-1 truncate" title={dealUrl(org?.handle ?? null, d.slug)}>
                {dealUrl(org?.handle ?? null, d.slug).replace(/^https?:\/\//, '')}
              </p>
